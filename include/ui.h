...
class texture;  // foward-declaration
...
enum e_alignment
{
  ...
  ALIGN_MASK_LEFT_TOP = tre::ui::ALIGN_MASK_HORIZONTAL_LEFT | tre::ui::ALIGN_MASK_VERTICAL_TOP,
};
...
struct s_layoutGrid
{
  ...
  glm::vec4 computeWidgetZones(const ui::window &win, const glm::vec2 &offset = glm::vec2(0.f)); ///< Computes the zone assigned to each widget. Returns the global zone.
  void clear();
};

struct s_layoutCircle
{

  void computeWidgetZones();
  void clear();
};
...
#define widget_DECLAREATTRIBUTE(wname,atype,aname,ainit) \
  public : \
    atype get_##aname() const { return w##aname; } \
    wname * set_##aname(atype a_##aname) { m_isUpdateNeededData |= (w##aname != a_##aname); w##aname = a_##aname; return this; } \
  protected :  \
    atype w##aname ainit;
...
{
public:
  widget() : wzone(0.f), m_isUpdateNeededData(false), m_parent(nullptr) {}
  virtual ~widget() {}

  /// @name attributes
  /// @{
  widget_DECLAREATTRIBUTE(widget,glm::vec4,color,= glm::vec4(0.8f, 0.8f, 0.8f, 1.f)) ///< main color
  widget_DECLAREATTRIBUTE(widget,bool,isactive,= false) ///< When true, the widget receives events and can trigger callbacks
  widget_DECLAREATTRIBUTE(widget,bool,iseditable,= false) ///< When true, the widget is editable from events
  widget_DECLAREATTRIBUTE(widget,bool,ishighlighted, = false) ///< read & write attribute: "acceptEvent" modifies the value; user can change the value, but no callback will be triggered in this case.
  /// @}
...
protected:
  ...
  virtual uint get_countText() const { return 0; } ///< Return nbr of text (a text as definied in the textgenerator)
  virtual const texture* get_textureSlot() const { return nullptr; } ///< Return texture for textued-drawing (a widget must have only 1 texture, excluding the font of the text)
  ...
  /// @{
public:
  bool getIsOverPosition(const glm::vec3 & position) const; ///< Return true if the position is under the wzone. Ignore position.z value.
...
protected:
  ...
  void set_zone(const glm::vec4 &assignedZone) { m_isUpdateNeededData |= (wzone != assignedZone); wzone = assignedZone; }

  glm::vec4  wzone;          ///< Widget zone (xmin,ymin,xmax,ymax)
  bool       m_isUpdateNeededData;

  struct s_objAdress
  {
    uint part;
    union { uint offset; uint pcount; };
  };
...
#define widget_DECLARECOMMUNMETHODS() \
  protected : \
    ...
    virtual const texture* get_textureSlot() const override; \
...
class widgetText : public widget
{
  ...
  widget_DECLAREATTRIBUTE(widgetText,std::string,text,) ///< Text to be drawn
  widget_DECLAREATTRIBUTE(widgetText,float,fontsizeModifier,= 1.f) ///< Font-size factor in regards with the parent window's font-size.
  widget_DECLAREATTRIBUTE(widgetText,bool,withborder,= false) ///< True if a border will be drawn
  widget_DECLAREATTRIBUTE(widgetText,bool,withbackground,= false) ///< True if a background will be drawn
...
public:
  ...
  widget_DECLAREATTRIBUTE(widgetTextEdit,bool,allowMultiLines,= false)
  widget_DECLAREATTRIBUTE(widgetTextEdit,float,cursorAnimSpeed,= 1.f)
...
class widgetPicture : public widget
{
  ...
  widget_DECLARECOMMUNMETHODS()
  widget_DECLAREATTRIBUTE(widgetPicture,texture*,texId,= nullptr)
  widget_DECLAREATTRIBUTE(widgetPicture,glm::vec4,texUV,= glm::vec4(0.f,0.f,1.f,1.f))
  widget_DECLAREATTRIBUTE(widgetPicture,bool,multiply,= true) ///< True if the color needs to be multiply with the widget's color
  widget_DECLAREATTRIBUTE(widgetPicture,float,heightModifier,= 1.f) ///< height factor in regards with the default cell's height.
};
...
class widgetBar : public widget
{
  ...
  widget_DECLARECOMMUNMETHODS()
  widget_DECLAREATTRIBUTE(widgetBar,float,valuemin,= 0.f)
  widget_DECLAREATTRIBUTE(widgetBar,float,valuemax,= 1.f)
  widget_DECLAREATTRIBUTE(widgetBar,float,value,= 0.f)
  widget_DECLAREATTRIBUTE(widgetBar,bool,withborder,= true) ///< True if a border must to be drawn
  widget_DECLAREATTRIBUTE(widgetBar,bool,withthreshold,= false)
  widget_DECLAREATTRIBUTE(widgetBar,float,valuethreshold,= 1.f)
  widget_DECLAREATTRIBUTE(widgetBar,bool,withtext,= false)
};
...
class widgetBoxCheck : public widget
{
  ...
  widget_DECLARECOMMUNMETHODS()
  widget_DECLAREATTRIBUTE(widgetBoxCheck,bool,value,= false)
};
...
class widgetLineChoice : public widget
{
  ...
  widget_DECLARECOMMUNMETHODS()
  widget_DECLAREATTRIBUTE(widgetLineChoice,std::vector<std::string>,values,)
  widget_DECLAREATTRIBUTE(widgetLineChoice,uint,selectedIndex,= 0)
  widget_DECLAREATTRIBUTE(widgetLineChoice,bool,cyclic,= false)
...
{
protected:
  window(tre::baseUI * parent) : widget(), m_parentUI(parent) { wcolor = glm::vec4(0.f); }
...
#define window_PROPERTY(atype,aname,ainit,uptodateFlag) \
  public: \
    const atype &get_##aname() const { return w##aname; } \
    void  set_##aname(atype a_##aname) { uptodateFlag |= (w##aname != a_##aname); w##aname = a_##aname; } \
  protected: \
    atype w##aname = ainit;
  ...
  window_PROPERTY(bool, snapPixels, true, m_isUpdateNeededLayout)
...
public:
  void set_layoutGrid(uint row, uint col) { m_isUpdateNeededLayout = true; wlayout.set_dimension(row, col); } ///< set the layout.
  ...
  void set_colAlignment(uint col, uint alignMask);
  void set_rowAlignment(uint row, uint alignMask);

  float     resolve_sizeW(s_size size) const;
  float     resolve_sizeH(s_size size) const;
  glm::vec2 resolve_sizeWH(s_size size) const;

protected:
  bool m_isUpdateNeededAdress = true; ///< true when object's adress update is needed (widget is created, ...)
  bool m_isUpdateNeededLayout = false; ///< true when layout update is needed (window is resized, widget gets resized, ...)
  //0}
...
public:
  ...
  glm::vec4 get_zone() { compute_layout(); return wzone; }
  /// @}
...
protected:
  ...
  void clear();
public:
  glm::vec2 snapToPixel(const glm::vec2 &windowCoord) const; ///< Snap the given point to the screen-pixel.
  glm::vec2 snapToPixel(const glm::vec2 &windowSize, const glm::ivec2 &pixelMin) const;  ///< Snap the given point to the screen-pixel, and clamp the min value
private:
  void compute_adressPlage(); ///< set adress-plage, and create parts in the m_model
  void compute_layout(); ///< compute and set zone of widgets
  s_layoutGrid  wlayout;
  widgetText* wTopBar = nullptr; ///< widget of the top-bar (the widget must inherit from "widgetText" class)
  widget* wCloseButton = nullptr;
  std::vector<const texture*> m_widgetTextureSlot; ///< Texture slots for the "pict" drawing.
...
#define window_DECLAREWIDGETHELPER(wname) \
  wname* get_##wname(uint row, uint col) \
  { \
    return static_cast<wname*>(get_widget(row, col)); \
  } \
  const wname* get_##wname(uint row, uint col) const \
  { \
    return static_cast<const wname*>(get_widget(row, col)); \
  } \
...
#undef window_DECLAREWIDGETHELPER
  ...
  widgetText* get_selfwidget_Title() const { return wTopBar; }
  widget* get_selfwidget_CloseBox() const { return wCloseButton; }

  void set_selfwidget_Title(widgetText * w);
...
{
public:
  baseUI() { eventState.mousePosPrev = glm::vec3(-20.f,-20.f,0.f); }
  virtual ~baseUI() { TRE_ASSERT(windowsList.empty()); TRE_ASSERT(m_shader == nullptr); TRE_ASSERT(m_whiteTexture == nullptr); }

  /// @name global settings
  /// @{
public:
  void set_defaultFont(const font * pfont) { m_defaultFont = pfont; }
...
protected:
  const font* m_defaultFont = nullptr; ///< Default font if not set in the child object.
...
protected:
  std::vector<ui::window*> windowsList; ///< List of windows; this class takes ownership of all windows.
  /// @}
...
public:
  ...
  void loadIntoGPU(); ///< First loading of data into GPU
